...
extern "C" GPU_results_t** initialize_results_str_in_GPU(size_t nDevices, size_t n, cuda_info_t *cuda_info, simulation_configuration_t *conf){

    size_t dev, dev_batch_size, time_steps;
    ...
    for(dev = 0; dev < nDevices; dev ++){
        ...
        if(conf->store_generated_spikes){
            err = cudaMalloc(&(tmp_r->gnt_spks), n * dev_batch_size * time_steps * sizeof(int));
            if (err != cudaSuccess) 
                printf("neuron_input_synapses_offset allocation failed: %s\n", cudaGetErrorString(err));
        }
...
extern "C" GPU_results_t** initialize_results_str_in_CPU(size_t nDevices, size_t n, cuda_info_t *cuda_info, simulation_configuration_t *conf){
...
extern "C" void cpy_batch_results_GPU2CPU(GPU_results_t *cpu_results, GPU_results_t *gpu_results, cuda_info_t *cuda_info, simulation_configuration_t *conf, size_t N, size_t dev){

    size_t batch_size = cuda_info->batch_size;
    size_t dev_batch_size, dev_batch_offset;
    size_t nDevices = cuda_info->nDevices;
    size_t time_steps = conf->time_steps;
    ...
    if(conf->store_generated_spikes)
        cudaMemcpy(cpu_results->gnt_spks, tmp_results->gnt_spks, N * dev_batch_size * time_steps * sizeof(int), cudaMemcpyDeviceToHost);

    free(tmp_results);
}

extern "C" void cpy_batch_results_multiGPU2CPU(GPU_results_t *cpu_results, GPU_results_t **gpu_results, cuda_info_t *cuda_info, simulation_configuration_t *conf, size_t N){
    ...
    for(dev = 0; dev < nDevices; dev ++){
        ...
        if(conf->store_generated_spikes)
            cudaMemcpy(cpu_results_per_dev->gnt_spks, tmp_results->gnt_spks, N * dev_batch_size * time_steps * sizeof(int), cudaMemcpyDeviceToHost);

        // copy to final structure
        if(conf->store_n_spikes || conf->store_generated_spikes){
            for(size_t i = 0; i<dev_batch_size; i++){
                ...
                for(size_t j = 0; j<N; j++){
                    ...
                    for(size_t t = 0; t<time_steps; t++){
                        ...
                        if(conf->store_generated_spikes)
                            cpu_results->gnt_spks[t * N * batch_size + j * batch_size + i + dev_batch_offset] = cpu_results_per_dev->gnt_spks[t * N * dev_batch_size + j * dev_batch_size + i];
                    }
                }
            }
        }
        ...
        if(conf->store_generated_spikes)
            free(cpu_results_per_dev->gnt_spks);
    }
...
__global__ void reinitialize_results_number_of_spikes_GPU(GPU_results_t *results, size_t N){
    ...
    if(threadId < N * dev_batch_size * time_steps){

        results->gnt_spks[threadId] = 0;
